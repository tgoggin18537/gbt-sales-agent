...
Last updated 2026-06-18. Source of truth: `worker/src/routes/webhook.ts`.

> **June 18 update (supersedes the June 16 note):** The takeover detector
> is now a source-driven classifier (`classifyConversationContext` in
> `ghl.ts`) that uses the GHL `messageType` + `source` fields, verified
> against real prod data. It fixed the bug where the bot shut ITSELF off
> mid-conversation: GHL returns "Opportunity created/updated" CRM activity
> rows as outbound messages, and those were being misread as a rep
> takeover. The bot now ignores activity rows + workflow emails, ignores
> workflow-sent texts (source=workflow is automation, never a takeover),
> and only treats a real human inbox text sent *after* the bot's last
> message as a takeover. The earlier `workflow-race-detected` tag was
> removed — it only existed because the old design couldn't tell
> automation from humans, which `source` now does cleanly. Same kill
> switch: `human-takeover` = bot off.
>
> **Diagnosis tool:** `GET /debug/classify?contactId=X` (auth header
> `x-ghl-webhook-secret`) returns the classifier's decision for any
> contact against live data, without sending anything. Use it to see
> exactly why a contact is/isn't getting a reply.
